...
function assign_from_description(f::SesquilinearForm)
   ...
   elseif f.descr==:hermitian f.X=GAP.Globals.HermitianFormByMatrix(f.mat_iso(gram_matrix(f)),f.ring_iso.codomain)
   else error("unsupported description")
   end
end
...
function Base.getproperty(f::SesquilinearForm, sym::Symbol)
   ...
   if sym === :mat_iso || sym === :ring_iso
      fm = gen_mat_iso(nrows(gram_matrix(f)), base_ring(gram_matrix(f)))
      f.ring_iso=fm.fr
      f.mat_iso=fm
